...
namespace marche
{
    internal class Program
    {
            ...
            var i18n = new Dictionary<string, string>()
        {
            ...
            /// MIB REDUSE
            ///////////////////////////////////////////////////////////////////////////////
            /* 1. Le nombre total de produits différents vendus (un produit est défini par son nom, peu importe le producteur) */
            var totalDifferentProducts = products.Select(p => p.name).Distinct().Count();
            /* 2. Le chiffre d’affaire possible **total** pour chaque marchand (tout produit confondu) */
            var totalRevenuePerProducer = products
            .GroupBy(p => p.producer)
            .Select(g => new
            {
                Producer = g.Key,
                TotalCA = g.Sum(p => p.quantity * p.price)
            });

            /* 3.le plus petit et la moyenne de ces chiffres d’affaire */
            var topProducer = products
            .GroupBy(p => p.producer)
            .Select(g => new
            {
                Producer = g.Key,
                minV = g.Min(p => p.quantity * p.price),
                avgV = g.Average(p => p.quantity * p.price)
            });

            /*3. Le marchand ayant le plus de noix à vendre*/
            var producerMaxN = products
            .Where(p => p.name == "Noix")
            .GroupBy(p => p.producer)
            .Select(g => new
            {
                Producer = g.Key,
                MaxN = g.Max(p => p.quantity)
            });
            ...
            var producerAffinity = products
            .Select(p => new
            {
                Producer = p.producer,
                Product = p.name,
                AffinityScore = Affinity(p.producer, p.name)
            });
             //afficher l'affinité de chaque producteur avec chaque produit
            
            var topAffinity = producerAffinity
            .OrderByDescending(pa => pa.AffinityScore)
            .First();
            Console.WriteLine($"Le marchand ayant le plus d'affinités avec ses produits est {topAffinity.Producer} avec un score de {topAffinity.AffinityScore}.");
