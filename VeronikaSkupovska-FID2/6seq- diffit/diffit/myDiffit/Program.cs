...
string? pathA = @"C:\Users\pu74vys\Documents\GitHub\323-Programmation_fonctionnelle\VeronikaSkupovska-FID2\6seq- diffit\diffit\v1.txt";  //Console.ReadLine();

Console.Write("Fichier B: ");
string? pathB = @"C:\Users\pu74vys\Documents\GitHub\323-Programmation_fonctionnelle\VeronikaSkupovska-FID2\6seq- diffit\diffit\v2.txt";  //Console.ReadLine();
...
{
    text = ignoreSpaces ? cleanSpaces(text) : text;
    text = ignoreTabs ? cleanTabs(text) : text;
    text = ignoreCase ? enforceCase(text) : text;
    return text;
};

linesA = linesA.Select(line => clean(line)).ToArray();
linesB = linesB.Select(line => clean(line)).ToArray();
